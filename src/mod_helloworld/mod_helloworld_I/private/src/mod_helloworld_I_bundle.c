...
/**
 * @file    mod_helloworld_I_bundle.c
...
static mod_iid_t mod_helloworld_iid = MOD_HELLOWORLD_IID;
...
{
	STD_ASSERT_RV(mod_register_instance_func(&mod_helloworld_iid, mod_helloworld_I_create_instance) == STD_RV_SUC,
		      STD_RV_ERR_FAIL);
	STD_ASSERT_RV(mod_bundle_IID_register(bundle_service_id, &mod_helloworld_iid) == STD_RV_SUC, STD_RV_ERR_FAIL);
...
{
	STD_ASSERT_RV(mod_unregister_instance_func(&mod_helloworld_iid) == STD_RV_SUC,);
