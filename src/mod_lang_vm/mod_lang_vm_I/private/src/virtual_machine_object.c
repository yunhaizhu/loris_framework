 ...
 * @return  STD_CALL std_char_t *
 */
STD_CALL std_char_t *get_object_type(IN own_object_t *obj)
{
    const object_symbol_t *symbol;
    std_char_t *ret = "";
    ...
    switch (obj->op) {
        case TYPE_I64_NUMBER:
            return "TYPE_I64_NUMBER";
        case TYPE_U64_NUMBER:
            return "TYPE_U64_NUMBER";
        case TYPE_BOOL:
            return "TYPE_BOOL";
        case TYPE_ADDRESS:
            return "TYPE_ADDRESS";
 ...
 * @return  STD_CALL std_bool_t
 */
STD_CALL std_bool_t equal_object(IN own_object_t *obj1, IN own_object_t *obj2)
{
    std_char_t buf1[BUF_SIZE_128] = "\0";
    std_char_t buf2[BUF_SIZE_128] = "\0";

    print_object_value_to_buf(obj1, buf1);
    print_object_value_to_buf(obj2, buf2);

    return strcmp(buf1, buf2) == 0 && std_safe_strlen(buf1, BUF_SIZE_128) == std_safe_strlen(buf2, BUF_SIZE_128);
}

/**
 * build_object_u64key_with_value
 * @brief   
 * @param   obj
 * @return  STD_CALL std_u64_t
 */
STD_CALL std_u64_t build_object_u64key_with_value(IN own_object_t *obj)
{
    std_char_t buf[BUF_SIZE_128] = "\0";
 ...
 * @param   token
 * @param   pub
 * @return  STD_CALL static inline fastcall std_u128_t
 */
STD_CALL static inline fastcall std_u128_t fast_encrypt(IN std_u16_t token, IN const struct public_key_class *pub)
{
    return token + 13 * pub->exponent + 27 * pub->modulus;
}
 ...
 * @return  STD_CALL std_bool_t
 */
STD_CALL std_bool_t create_ownership_signature(IN const object_symbol_t *owner_symbol, IN own_object_t *need_obj)
{
    STD_ASSERT_RV_WARN(need_obj->owner_token_signature.token[0] == 0, STD_BOOL_FALSE);
 ...
 * @return  STD_CALL std_bool_t
 */
STD_CALL std_bool_t verify_ownership_signature(IN const object_symbol_t *owner_symbol, IN const own_object_t *need_obj)
{
    STD_ASSERT_RV_WARN(need_obj != NULL, STD_BOOL_FALSE);
    STD_ASSERT_RV_WARN(need_obj->owner_token_signature.token[0] != 0, STD_BOOL_FALSE);
    STD_ASSERT_RV_WARN(need_obj->owner_token_signature.signature[0] != 0, STD_BOOL_FALSE);
 ...
 * @return  STD_CALL std_bool_t
 */
STD_CALL std_bool_t clear_ownership_signature(IN const object_symbol_t *owner_symbol, IN own_object_t *need_obj)
{
    STD_ASSERT_RV_WARN(verify_ownership_signature(owner_symbol, need_obj) == STD_BOOL_TRUE, STD_BOOL_FALSE);
...
/**
 * free_ownership_object
 * @brief
 * @param   owner_symbol
 * @param   item
 * @return  STD_CALL std_rv_t
 */
STD_CALL std_rv_t free_ownership_object(IN const object_symbol_t *owner_symbol, IN own_object_t *item)
{
    STD_ASSERT_RV_WARN(verify_ownership_signature(owner_symbol, item) == STD_BOOL_TRUE, STD_RV_ERR_FAIL);
    STD_ASSERT_RV_WARN(item != NULL, STD_RV_ERR_POINTER);
    ...
    switch (item->op) {
        ...
        case TYPE_ADDRESS:
            ...
            break;
        case TYPE_STRING:
            STD_LOG(DEBUG, "FREE STR %p %p %s \n", item, item->value.string, item->value.string);
            FREE(item->value.string);
            FREE(item);

            break;
        case TYPE_SYMBOL:
            if (get_object_symbol(item) == NULL) {
                break;
            }
            STD_LOG(DEBUG, "FREE %s %s %p %p\n", __FUNCTION__, item->value.symbol->name, item, item->value.symbol);
            switch (get_object_symbol(item)->env_value.symbol_type) {
                case var_type:
                    del_VAR_with_var_type(item->value.symbol);
                    break;

                case array_type:
                    del_VARS_with_array_type(item->value.symbol);
                    break;

                case tuple_type:
                    del_VARS_with_tuple_type(item->value.symbol);
                    break;

                case hash_type:
                    del_VARS_with_hash_type(item->value.symbol);
                    break;
 ...
 * @return  STD_CALL std_rv_t
 */
STD_CALL std_rv_t free_object(IN own_object_t *item)
{
    STD_ASSERT_RV_WARN(item != NULL, STD_RV_ERR_POINTER);
    STD_ASSERT_RV_WARN(item->owner_token_signature.signature[0] == 0, STD_RV_ERR_UNEXPECTED);
...
/**
 * inline_create_ownership_object
 * @brief
 * @param   item
 * @return  STD_CALL static inline object_t *
 */
STD_CALL static inline own_object_t *inline_create_ownership_object(IN const own_object_t *item)
{
    own_object_t *copy = NULL;

    copy = (own_object_t *) CALLOC(sizeof(own_object_t), 1);
    memcpy(copy, item, sizeof(own_object_t));
    memset(&copy->list, 0, sizeof(copy->list));
    for (int i = 0; i < 4; ++i) {
        copy->owner_token_signature.token[i] = 0;
        copy->owner_token_signature.signature[i] = 0;
    }
    return copy;
}

/**
 * inline_create_ownership_symbol
 * @brief   
 * @param   item
 * @return  STD_CALL static inline object_t *
 */
STD_CALL static inline own_object_t *inline_create_ownership_symbol(IN own_object_t *item)
{
    own_object_t *copy = NULL;
    std_char_t *name;
    std_char_t key[BUF_SIZE_32] = "\0";

    copy = inline_create_ownership_object(item);
    copy->value.symbol = (object_symbol_t *) CALLOC(sizeof(object_symbol_t), 1);
    memcpy(copy->value.symbol, item->value.symbol, sizeof(object_symbol_t));
    name = (std_char_t *) CALLOC(sizeof(char), BUF_SIZE_128);
    snprintf(name, BUF_SIZE_128, "%s-MC", item->value.symbol->name);
    copy->value.symbol->name = name;

    //move item's private public key to copy.
    rsa_gen_keys(&item->value.symbol->pub, &item->value.symbol->pri, PRIME_SOURCE_FILE);

    snprintf(key, sizeof(key), "local_%s_%p", name, copy);
    std_lock_free_key_hash_add(vm[get_std_thread_id()].symbol_hash, key, std_safe_strlen(key, sizeof(key)), copy);

    return copy;
}

/**
 * check_support_object_type
 * @brief   
 * @param   item
 * @return  STD_CALL std_bool_t
 */
STD_CALL std_bool_t check_support_object_type(IN const own_object_t *item)
{
    switch (item->op) {
        case TYPE_I64_NUMBER:
        case TYPE_U64_NUMBER:
 ...
 * @return  STD_CALL object_t *
 */
STD_CALL own_object_t *create_ownership_object(IN const object_symbol_t *owner_symbol, IN own_object_t *item)
{
    own_object_t *copy = NULL;

    STD_ASSERT_RV(item != NULL, NULL);
    ...
    switch (item->op) {
        ...
        case TYPE_ADDRESS:
            copy = inline_create_ownership_object(item);
            create_ownership_signature(owner_symbol, copy);
            break;

        case TYPE_STRING:
            copy = inline_create_ownership_object(item);
            copy->value.string = strdup(item->value.string);
            create_ownership_signature(owner_symbol, copy);
            STD_LOG(DEBUG, "ALLOC STR %p %p %s\n", copy, copy->value.string, copy->value.string);
            break;
        ...
        case TYPE_SYMBOL:
            ...
            switch (get_object_symbol(item)->env_value.symbol_type) {
                case var_type:
                    copy = inline_create_ownership_symbol(item);
                    move_VAR_with_var_type(item, get_object_symbol(copy));
                    create_ownership_signature(owner_symbol, copy);
                    break;

                case array_type:
                    copy = inline_create_ownership_symbol(item);
                    move_VAR_with_array_type(item, get_object_symbol(copy));
                    create_ownership_signature(owner_symbol, copy);

                    STD_LOG(DEBUG, "ALLOC %s %s %p %p\n", __FUNCTION__, copy->value.symbol->name, copy, copy->value.symbol);
                    break;

                case tuple_type:
                    copy = inline_create_ownership_symbol(item);
                    move_VAR_with_tuple_type(item, get_object_symbol(copy));
                    create_ownership_signature(owner_symbol, copy);
                ...
                case hash_type:
                    copy = inline_create_ownership_symbol(item);
                    move_VAR_with_hash_type(item, get_object_symbol(copy));
                    create_ownership_signature(owner_symbol, copy);
...
/**
 * add_auto_free_object
 * @brief   
 * @param   obj
 * @return  STD_CALL std_void_t
 */
STD_CALL std_void_t add_auto_free_object(own_object_t *obj)
{
    std_char_t key[BUF_SIZE_128] = "\0";
    STD_ASSERT_RV_WARN(obj!= NULL, );

    if (create_ownership_signature(&vm[get_std_thread_id()].global_system_object_symbol, obj) == STD_BOOL_TRUE){
        snprintf(key, sizeof(key), "auto_free_%p", obj);
        std_lock_free_key_hash_add(vm[get_std_thread_id()].auto_free_hash, key, std_safe_strlen(key, sizeof(key)), obj);
    }
}
 ...
 * @return  STD_CALL std_void_t
 */
STD_CALL std_void_t print_object_value(IN own_object_t *obj)
{
    own_object_t *value = obj;

    if (value && value->op == TYPE_SYMBOL) {
        value = get_VAR(value, NULL, STD_BOOL_FALSE);
    }
    if (value == NULL) {
        STD_LOG(DISPLAY, "null ");
        return;
    ...
    if (value->op == TYPE_I64_NUMBER) {
        STD_LOG(DISPLAY, "%ld ", get_object_number(value));
    } else if (value->op == TYPE_U64_NUMBER){
        STD_LOG(DISPLAY, "%lu ", get_object_u_number(value));
    } else if (value->op == TYPE_STRING) {
        STD_LOG(DISPLAY, "%s ", get_object_string(value));
    } else if (value->op == TYPE_ADDRESS) {
        STD_LOG(DISPLAY, "%p ", get_object_address(value));
    } else if (value->op == TYPE_BOOL) {
        STD_LOG(DISPLAY, "%s ", get_object_bool(value)?"true":"false");
    }else if (value->op == TYPE_SYMBOL) {
        STD_LOG(DISPLAY, "%s ", get_object_type(value));
    }
}
 ...
 * @return  STD_CALL std_void_t
 */
STD_CALL std_void_t print_object_value_to_buf(IN own_object_t *obj, IN std_char_t *buf)
{
    if (obj == NULL) {
        snprintf(buf, BUF_SIZE_128, "%u", 0);
        return;
    }

    if (obj->op == TYPE_I64_NUMBER) {
        snprintf(buf, BUF_SIZE_128, "%ld", get_object_number(obj));
    } else if (obj->op == TYPE_U64_NUMBER){
        snprintf(buf, BUF_SIZE_128, "%lu", get_object_u_number(obj));
    } else if (obj->op == TYPE_BOOL) {
        snprintf(buf, BUF_SIZE_128, "%d", get_object_bool(obj));
    } else if (obj->op == TYPE_STRING) {
        snprintf(buf, BUF_SIZE_128, "%s", get_object_string(obj));
    } else if (obj->op == TYPE_SYMBOL) {
        own_object_t *value = get_VAR(obj, NULL, STD_BOOL_FALSE);
        print_object_value_to_buf(value, buf);
    }
}
#endif
