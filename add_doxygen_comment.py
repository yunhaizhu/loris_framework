...
gb_func_reg_ex = '^\s*(?:\s+)?([A-Za-z_][\sA-Za-z_0-9\*&:]*)\\b(' + ident_reg_ex + ')\s*\((.*)\)\s*(?:const)?\s*$'

gb_func_reg_ex2 = '^\s*(?:\s+)?([A-Za-z_][\sA-Za-z_0-9\*&:]*)\\b(' + ident_reg_ex + ')\s*\((.*)\)\s*(?:const)?\s*'

key_words = ['do', 'if', 'return', 'typedef', 'double', 'short', 'bool', 'int', 'signed', 'break', 'else', 'long',
             'sizeof', 'union', 'case', 'enum', 'static', 'unsigned', 'catch', 'namespace', 'using', 'char', 'export',
             'new', 'struct', 'virtual', 'class', 'extern', 'operator', 'switch', 'void', 'const', 'false', 'private',
             'template', 'float', 'protected', 'this', 'continue', 'for', 'include', 'throw', 'while', 'friend', 'true']
...
def do_head_file(file_name):
    ...
    for line in lines_in:
        ...
        # we should ignore the functions inside of comment
        if not is_in_comment:
            t, c, f, a, r = get_func_in_line2(line)
            if t != '':
                # check wether function need comment
                if is_need_insert_comment(lines_in, index):
                    lines_out += make_func_comment(t, c, f, a, r)
